	...
	FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (FPhysicalObjectTrackingUtility::GetTrackedDevicePositionAndRotation(m_CurrentTargetDeviceId, trackedPosition, trackedOrientation))
	{
		FTransform trackerFromReference;
		if (m_Reference != nullptr)
		{
			FQuat orientation = trackedOrientation * m_Reference->GetNeutralRotationInverse();
			FVector position = trackedPosition - m_Reference->GetNeutralOffset();
	...
	else
	{
		...
		UE_LOG(LogPhysicalObjectTracker, Warning, TEXT("Failed to acquire TrackedDevicePosition for device id %i"), m_CurrentTargetDeviceId);
	}
...
void UPhysicalObjectTrackingComponent::DebugCheckIfTrackingTargetExists() const
{
	...
	if (!deviceIds.Contains(m_CurrentTargetDeviceId))
	{
		TWideStringBuilder<4096> builder{};
		builder.Appendf(TEXT("Could not find SteamVR Controller with DeviceID: %i. Valid device IDs are: "), m_CurrentTargetDeviceId);
